...
__all__ = ['open_start_page', 'open_auth_page', 'sign_in', 'open_main_page', 'logout']
...
def open_start_page(app: Application, request: FixtureRequest) -> None:
    with allure.step('Opening Start page'):
        if request.config.option.block_urls:
            app.send_command('Network.setBlockedURLs', {'urls': ['www.google.com']})
...
def open_auth_page(app: Application) -> None:
    with allure.step('Opening Auth page'):
        try:
            StartPage(app).header.lk_user.click()
